...
GLboolean ESUTIL_API esCreateWindow ( ESContext *esContext, const char* title, GLint width, GLint height, GLuint flags )
{
   EGLint attribList[] =
   {
       ...
       EGL_DEPTH_SIZE,     24,//(flags & ES_WINDOW_DEPTH) ? 8 : EGL_DONT_CARE,
...
void ESUTIL_API esRegisterKeyDownFunc ( ESContext *esContext,
                                    void (ESCALLBACK *keyDownFunc) (ESContext*, unsigned char, int, int ) )
{
   esContext->keyDownFunc = keyDownFunc;
}

void ESUTIL_API esRegisterKeyUpFunc ( ESContext *esContext,
                                    void (ESCALLBACK *keyUpFunc) (ESContext*, unsigned char, int, int ) )
{
   esContext->keyUpFunc = keyUpFunc;
}
